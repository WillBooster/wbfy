...
1. Read the PR body and its messages with `fetch-pr` skill.
2. Check the results of CI (GitHub Actions) with `check-pr-ci` skill.
3. If any workflow has failed, fix the CI issues, commit, push, and then return to step 2. Otherwise, go to step 4.
4. Fetch unresolved review threads with `manage-pr-review-threads` skill.
...
7. Reply to all review threads with `manage-pr-review-threads` skill.
