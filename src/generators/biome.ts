...
export async function generateBiomeJsonc(config: PackageConfig): Promise<void> {
  return logger.functionIgnoringException('generateBiomeJsonc', async () => {
    ...
    try {
      ...
      const oldSettings = JSON.parse(oldContent) as BiomeSettings;
    ...
    newSettings = {
      ...
      files: {
        ...newSettings.files,
        includes: getBiomeIncludes(newSettings.files?.includes),
      },
    };
...
function getBiomeIncludes(existingIncludes: string[] | undefined): string[] {
  return [...new Set([...(existingIncludes ?? []), '**', '!!.agents'])];
}
