...
export async function installAgentSkills(rootConfig: PackageConfig): Promise<void> {
  return logger.functionIgnoringException('installAgentSkills', async () => {
    // TODO: We are temporally remove agent skills on repo
    // await runInstallAgentSkills(rootConfig);
    await fs.rm(path.resolve(rootConfig.dirPath, '.agents'), {
      force: true,
      recursive: true,
    });
    await fs.rm(path.resolve(rootConfig.dirPath, '.claude'), {
      force: true,
      recursive: true,
    });
    await fs.rm(path.resolve(rootConfig.dirPath, 'skills-lock.json'), {
      force: true,
    });
  });
}
